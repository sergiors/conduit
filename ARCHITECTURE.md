...
- **Idempotency is best-effort and bounded by the Redis processed-key TTL.** The processed key (`cdc:processed:{id}`) suppresses duplicate deliveries only within its 24h TTL window. After it expires, a replayed event is delivered again.
...
- **Idempotency is required for all event processing**. Duplicate event IDs within the 24h processed-key TTL are skipped. Idempotency is best-effort and bounded by that TTL; delivery is at-least-once, so downstream consumers must be idempotent using `eventID`.
...
- `cmd/main.go`: A single entrypoint that dispatches to the CLI commands via `internal/cli`, which selects between `start`, `health`, and `apikey`. Runtime logic lives in the packages below rather than in main. main owns process boundary concerns only: LOG_LEVEL parsing, the tint/slog logger, the signal-aware root context, CLI invocation, error printing, and the exit code.
- `internal/cli`: Command selection, usage text, and argument handling; each command is a thin wrapper over the underlying runtime packages. `conduit start` loads the config and delegates to `internal/runtime` — no application lifecycle logic lives here.
- `internal/runtime`: The application composition root. It creates the shared infrastructure — a single MongoDB client (one connection pool for the whole process) and a single Redis client, plus the shared `collections.Manager` wired with the Redis Pub/Sub/purge hooks and the indexes — then creates the API and worker components and coordinates their lifecycle: both run concurrently in the same process, a shared derived context drives graceful shutdown on SIGINT/SIGTERM, a failure in either component cancels the other so it shuts down gracefully, and the first error is propagated to the exit boundary. Shared clients are closed by the runtime (Redis first, then MongoDB) only after their consumers have stopped. Configuration is passed in and no business logic lives here.
- `internal/api`: The HTTP surface — Gin routes, middleware, and handlers. The server is constructed with injected dependencies (`Server.Start(ctx, cfg, logger)` serves and performs a graceful HTTP shutdown bounded by `SHUTDOWN_TIMEOUT`); it neither creates nor closes shared clients.
- `internal/worker`: Hosts the CDC worker runtime. Shared infrastructure (MongoDB client, Redis client, collections manager) is injected by the composition root; the worker owns and creates its dispatcher, retry processor, watcher manager, and the metrics stack, with an explicit Start/Shutdown lifecycle.

  **Graceful shutdown.** On SIGINT or SIGTERM, propagated through the composition root's shared context, the worker shuts down in dependency order, bounded by `SHUTDOWN_TIMEOUT` (default 30s): (1) the watcher manager is stopped first — cancelling its run context, waiting for its sync/config-change loops and every watcher, and closing pub/sub — so no new events flow while in-flight bookkeeping drains; (2) the retry processor is stopped, letting the current pass finish; (3) the metrics refresher/logger are stopped; (4) the dispatcher stops every sink lane (waiting for in-flight deliveries to drain) and closes the transports; (5) the metrics server is stopped last so `/metrics` serves through the drain. The shared Redis and MongoDB clients are NOT closed by the worker — they are injected and closed by the composition root after all consumers have stopped. Terminal bookkeeping writes (resume-token persist, `MarkProcessed`, retry `Enqueue`/`Remove`) and change-stream cursor close use a short detached context so a mid-flight event is never lost when the live context is cancelled. No arbitrary sleeps are used; shutdown is driven entirely by context cancellation and `sync.WaitGroup` waits. Panics in worker goroutines are contained: each long-running goroutine has a recover backstop that logs with a stack trace, per-event/per-tick work is panic-isolated so a single bad event cannot kill its loop, and a panicking watcher marks itself stopped for the manager's sync to reconcile.
