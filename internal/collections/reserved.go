...
// config.collections holds collection settings and config.sinks holds sink
// settings. Creating a CDC collection with one of these names would collide
// with Conduit's own control-plane storage.
...
// configuration namespaces (config.collections, config.sinks) and MongoDB's
// system namespaces (system.*), which MongoDB itself reserves for internal
// use. The check is exact and case-sensitive, matching how MongoDB treats
// collection names.
