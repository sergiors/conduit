...
func (c *Client) DropTTLIndex(ctx context.Context, collection, field string) error {
	name := field + "_1"
	idxs, err := c.Collection(collection).Indexes().ListSpecifications(ctx)
	if err != nil {
		return fmt.Errorf("list indexes: %w", err)
	}
	for _, idx := range idxs {
		if idx.Name == name {
			if _, err := c.Collection(collection).Indexes().DropOne(ctx, name); err != nil {
				return fmt.Errorf("drop ttl index: %w", err)
			}
			return nil
		}
	}
	return nil // index not present, nothing to drop
}
